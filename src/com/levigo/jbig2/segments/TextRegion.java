...
public class TextRegion implements Region {
	...
	private Map<Number, List<Point>> positions = new HashMap<Number, List<Point>>();
	...
	private void readRegionFlags() throws IOException {
		...
		combinationOperator = CombinationOperator
				.translateOperatorCodeToEnum((short) (subInputStream
						.readBits(2) & 0x3));
	...
	public List<Bitmap> getSymbolLists() {
		return symbols;
	}

	public Map<Number, List<Point>> getPositions() {
		return positions;
	}
	...
	private void computeSymbolCodeLength() throws IOException {
		if (isHuffmanEncoded) {
			...
			symbolCodeLength = (int) Math
					.ceil((Math.log(amountOfSymbols) / Math.log(2)));
		}
	}
	...
	private final long decodeStripT() throws IOException,
		...
		if (isHuffmanEncoded) {
			...
			if (sbHuffDT == 3) {
				...
				stripT = StandardTables.getTable(11 + sbHuffDT).decode(
						subInputStream);
			}
		} else {
	...
	private final long decodeDT() throws IOException {
		...
		if (isHuffmanEncoded) {
			if (sbHuffDT == 3) {
				...
				dT = StandardTables.getTable(11 + sbHuffDT).decode(
						subInputStream);
			}
		} else {
	...
	private final long decodeDfS() throws IOException,
		...
		if (isHuffmanEncoded) {
			if (sbHuffFS == 3) {
				...
				return StandardTables.getTable(6 + sbHuffFS).decode(
						subInputStream);
			}
		} else {
	...
	private final long decodeIdS() throws IOException,
		...
		if (isHuffmanEncoded) {
			if (sbHuffDS == 3) {
				...
				return StandardTables.getTable(8 + sbHuffDS).decode(
						subInputStream);
			}
		} else {
	...
	private final Bitmap decodeIb(long r, long id) throws IOException,
		...
		if (r == 0) {
			...
			if (genericRefinementRegion == null) {
				genericRefinementRegion = new GenericRefinementRegion(
						subInputStream);
			}

			genericRefinementRegion.setParameters(cx, arithmeticDecoder,
					sbrTemplate, (int) (wo + rdw), (int) (ho + rdh), ibo,
					genericRegionReferenceDX, genericRegionReferenceDY, false,
					sbrATX, sbrATY);
	...
	private final long decodeRdw() throws IOException,
		...
		if (isHuffmanEncoded) {
			if (sbHuffRDWidth == 3) {
				...
				return StandardTables.getTable(14 + sbHuffRDWidth).decode(
						subInputStream);
			}
		} else {
	...
	private final long decodeRdh() throws IOException,
		...
		if (isHuffmanEncoded) {
			if (sbHuffRDHeight == 3) {
				...
				return StandardTables.getTable(14 + sbHuffRDHeight).decode(
						subInputStream);
			}
		} else {
	...
	private final long decodeRdx() throws IOException,
		...
		if (isHuffmanEncoded) {
			if (sbHuffRDX == 3) {
				...
				return StandardTables.getTable(14 + sbHuffRDX).decode(
						subInputStream);
			}
		} else {
	...
	private final long decodeRdy() throws IOException,
		...
		if (isHuffmanEncoded) {
			if (sbHuffRDY == 3) {
				...
				return StandardTables.getTable(14 + sbHuffRDY).decode(
						subInputStream);
			}
		} else {
	...
	private final void blit(Bitmap ib, long t, long id) {
		if (isTransposed == 0 && (referenceCorner == 2 || referenceCorner == 3)) {
			currentS += ib.getWidth() - 1;
		} else if (isTransposed == 1
				&& (referenceCorner == 0 || referenceCorner == 2)) {
		...
		List<Point> points = positions.get(id);
		if (points == null) {
			points = new ArrayList<Point>();
			positions.put(id, points);
		}
		points.add(new Point((int) s, (int) t));
