...
class JBIG2Page {
	...
	private HashMap<Number, ArrayList<Object>> symbols = new HashMap<Number, ArrayList<Object>>();
	...
	protected JBIG2Page(JBIG2Document document, int pageNumber) {
		...
		this.symbols = new HashMap<Number, ArrayList<Object>>();
	}
	 ...
	 * 
	 * @param number
	 *            - Segment number of the segment to search.
	 * 
	 * @return The retrieved {@link SegmentHeader} or {@code null} if not
	 *         available.
	 */
	 ...
	 * 
	 * @return The associated {@link PageInformation} segment or {@code null} if
	 *         not available.
	 */
	...
	private void createNormalPage(PageInformation pageInformation)
		...
		pageBitmap = new Bitmap(pageInformation.getWidth(),
				pageInformation.getHeight());
	...
	private void collectSymbols(Region r) {
		if (r instanceof TextRegion) {
			ArrayList<Object> symbolList = symbols.get(r.hashCode());
			if (symbolList == null) {
				symbolList = new ArrayList<Object>();
				symbols.put(r.hashCode(), symbolList);
			}
			symbolList.add(((TextRegion) r).getSymbolLists());
			symbolList.add(((TextRegion) r).getPositions());
			symbolList.add(((TextRegion) r).getRegionInfo());
		}
	}

	/**
	 * Check if we have only one region that forms the complete page. If the
	 * dimension equals the page's dimension set the region's bitmap as the
	 * page's bitmap. Otherwise we have to blit the smaller region's bitmap into
	 * the page's bitmap (see Issue 6).
	 * 
	...
	/**
	 * This method counts the regions segments. If there is only one region, the
	 * bitmap of this segment is equal to the page bitmap and blitting is not
	 * necessary.
	 * 
	...
	public HashMap<Number, ArrayList<Object>> getSymbols() {
		return symbols;
	}
